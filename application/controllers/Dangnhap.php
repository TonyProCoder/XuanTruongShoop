...
class Dangnhap extends CI_Controller {
    ...
    public function dangnhap(){
        ...
        if($this->form_validation->run() ==TRUE){
            ...
            $this->data['title']='XuanTruongShop - Đăng nhập tài khoản';
            $this->data['view']='dangnhap';
            $this->load->view('frontend/layout',$this->data);
        }     
    }
    ...
    public function dangky(){
        ...
        if($this->form_validation->run() ==TRUE){
            ...
            $config['smtp_user']    = '99999truong@gmail.com';
            $config['smtp_pass']    = 'gpof kwpm cuif qmda';
            ...
            $this->email->from('99999truong@gmail.com', 'XuanTruongShop');
            $this->email->to($email);
            $this->email->subject('Hệ thống Xuân Trường Shop - Quà thành viên mới');
            $this->email->message('Bạn đã trở thành thành viên mới của cửa hàng Xuân Trường Shop, Cửa hàng tặng bạn 1 mã giảm giá giảm 100.000 đ : '.$tempcoupon.' , Mã này có giá trị tới ngày '.$tempdatelimit.'
                Hãy sử dụng tài khoản để mua hàng để tích lũy nhận thêm nhiều ưu đãi !!!!');
            $this->email->send();
            $this->data['success']='Đăng ký thành công! Bạn đã nhận được 1 mã giảm giá cho thành viên mới, vui lòng kiểm tra email của mình nhé !!';


        }  
        $this->data['title']='XuanTruongShop - Đăng ký tài khoản';   
...
//laylaipass
    public function forget_password(){
        ...
        if($this->form_validation->run() ==TRUE){
            ...
            $config['smtp_port']    = '465';
            $config['smtp_timeout'] = '7';
            $config['smtp_user']    = 'quantrivien.cntt.99999@gmail.com';
            $config['smtp_pass']    = 'eqdc zneg nqne jhza';
            $config['charset']    = 'utf-8';
            $config['newline']    = "\r\n";
            $config['wordwrap'] = TRUE;
            $config['mailtype'] = 'html';
            $config['validation'] = TRUE;   
            $this->email->initialize($config);
            $this->email->from('XuanTruongShop', 'XuanTruongShop');
            $this->email->to($list['email']);
            $this->email->subject('XuanTruongShop - Lấy lại mật khẩu');
            ...
            $this->email->send();
            $this->data['success']='Bạn vui lòng kiểm tra email để lấy lại mật khẩu!';   
        }  
        $this->data['title']='XuanTruongShop - Quên mật khẩu';   
    ...
    public function reset_password_new($id){
        ...
        if($this->form_validation->run() ==TRUE){ 
           ...
           if($this->Mcustomer->customer_check_id_email($id, $email)!=FALSE){
            ...
            $this->data['title']='XuanTruongShop - Cập nhật mật khẩu mới';
